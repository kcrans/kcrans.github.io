...
During the inline processing phase of interpatation, character sequences that match the regex `\[\^s\s(.*?)\]`[^s i.e. `[^s This is a sidenote ]` ] will be converted into the appropaite html for sidenotes as specified by tufte-css.

## Deployment

Deployment on Github Pages with Pelican is just as easy as setting up a Github action like this [one](https://docs.getpelican.com/en/latest/tips.html#publishing-to-github-pages-using-a-custom-github-actions-workflow). It takes about 45 seconds to deploy with the configuration I have. Make sure to include markdown as a dependency if you are using it for writitng post like I am.
